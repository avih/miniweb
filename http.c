...
"    var d = files[i][1] == -1;  // is a directory\n"
"    function mkhref(f) { return !d ? f[0] : (f[0] == '..' ? '../' : f[0]) + diro; }\n"
"    out += '<tr>'\n"
"         + '<td align=left ><a href = \"' + mkhref(files[i]) + '\">' + files[i][0] + '</a></td>'\n"
"         + '<td align=right>' + (d ? '' : files[i][1]) + '</td>'\n"
"         + '<td align=right>' + files[i][2] + '</td>'\n"
"         + '<td            >' + (new Date(1000 * files[i][3])).toISOString() + '</td>'\n"
"         + '</tr>';\n"
"  };\n"
"  document.write(out);\n"
"}\n";
...
int _mwListDirectory(HttpSocket* phsSocket, char* dir)
{
	...
	_mwListDirectory_internal(phsSocket, dir, 0);
	hpf(phsSocket, "</noscript>\n");

	hpf(phsSocket, "<script>\n%s\nwriteHead();\nvar files=\"", dir_list_js);
	_mwListDirectory_internal(phsSocket, dir, 1);
