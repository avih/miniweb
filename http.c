...
int _mwListDirectory_internal(HttpSocket* phsSocket, char* dir, int isscript)
{
	char cFileName[128];
	...
	for (ret=ReadDir(dir,cFileName); !ret; ret=ReadDir(NULL,cFileName)) {
		...
		hpf(phsSocket, "<tr><td><a href='%s%s'>%s</a></td><td align=right>",
			cFileName, (isdir ? "/" : ""), cFileName);
		if (!isdir)
			hpf(phsSocket, "%u KB", (unsigned int)(st.st_size >> 10));
		hpf(phsSocket, "</td><td align=right>");
		if (isdir) {
			hpf(phsSocket, "&lt;dir&gt;");
		} else if (ext = strrchr(cFileName,'.')) {
			int filetype=mwGetContentType(++ext);
			if (filetype!=HTTPFILETYPE_OCTET)
				hpf(phsSocket, "%s", contentTypeTable[filetype]);
			else
				hpf(phsSocket, "%s file",ext);
		}
		hpf(phsSocket, "</td><td>");
		mwGetHttpDateTime(st.st_mtime, tmp, 511); hpf(phsSocket, "%s", tmp);
		hpf(phsSocket, "</td></tr>");
	}
...
const char *script = ""
"var order = document.location.href.split('?')[1] || '';\n"
"function sortLink(name) {\n"
"  return '<a href=\"?' + name[0] +\n"
"         (name[0] != order[0] ? '' : order[1] ? '' : 'd') + '\">' +\n"
"         name + '</a>';\n"
"}\n"
"function writeHead() {\n"
"  document.write('<tr><th align=left >' + sortLink(\"Name\") + '</th>' +\n"
"                     '<th align=right>' + sortLink(\"Size\") + '</th>' +\n"
"                     '<th align=right>' + sortLink(\"Type\") + '</th>' +\n"
"                     '<th            >' + sortLink(\"Modified\") + '</th></tr>')\n"
"}";

int _mwListDirectory(HttpSocket* phsSocket, char* dir)
{
	char webpath[512] = {0};
	strncpy(webpath, phsSocket->request.pucPath, 511);
	if (strchr(webpath, '?')) *strchr(webpath, '?') = 0;
	if (webpath[0]) webpath[strlen(webpath) - 1] = 0;
	hp_init(phsSocket);
	hpf(phsSocket, "<html><head><title>/%s</title></head>"
	               "<style>\nth, td{padding: 0 20px 0 0; white-space:pre;}\n</style>"
	               "<script>\n%s\n</script>"
	               "</head><body><table style='border: 0px;'><h2>Directory of /%s</h2>",
	    webpath, script, webpath);
	hpf(phsSocket, "<script>writeHead();</script>");
	return _mwListDirectory_internal(phsSocket, dir, 0);
}
